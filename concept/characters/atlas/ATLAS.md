# ATLAS - character bible (the penitent giant / tank)

> Built via `concept/CHARACTER-METHOD.md`. **ESSENCE (layers 0-3, 7-9) LOCKED with Aviv
> 2026-07-03. SURFACE (layers 4-6: body material, palette, signature object) is DRAFT and
> DELIBERATELY OPEN - Aviv is bringing visual references to shape the look. NO generation
> until those references land.**

Archetype: **TANK** (high HP, slow, defensive, knockback-shove). The third roster pole -
FANG = reckless speed (fire, grounded), ZERO = cold control (cosmic ice, floats),
**ATLAS = enduring protection (immense, grounded, immovable).**

---

## 0. THE PITCH / PLAYER FANTASY
**Be the unbreakable wall.** You don't chase and you don't dodge - you plant, you endure, and
everything that hits you breaks first. The immovable protector: the enemy cannot get past you,
and the longer the fight goes, the more it's yours. For the player who wants to outlast, absorb,
and win by refusing to fall.

## 1. THE SEED + THE CONTRADICTION (the heart)
A colossal **guardian statue** that woke with a conscience. **The most destructive body in the
arena, driven by a heart that refuses to destroy.** He was raised to PROTECT - and he was made
to level the very thing he guarded. Everything downstream hangs on this single tension:
destruction incarnate, wielded by someone who now only wants to shield.

## 2. THE WOUND & THE WANT
- **Wound:** he destroyed what he was built to protect. (Turned/used as a weapon against his own
  charge - the exact mechanism is open, but the guilt is the engine.) He carries every stone of
  it.
- **Want (his OVERTHRONE wish - LOCKED):** **to restore what he destroyed.** The unclaimed wish
  is the only thing in the world big enough to undo what he did. That is why a being who hates to
  fight enters a fighting league at all - it is the one door back.
- **Fear:** being made to destroy again. Losing control a second time.

## 3. THE WAY THEY FIGHT (bridge to mechanics - LOCKED)
He fights **purely defensively, on purpose.** He absorbs, he endures, he **shoves you back
("you shall not pass") but never strikes to kill.** A fight, to ATLAS, is not a contest - it is
**penance**: hold the line, take the blows, prove a weapon can choose to only guard. His skill /
stance is a wall, not a weapon.
- **Origin of skill:** not trained, not instinct - **built.** He is a made thing whose every
  design was for force; his discipline is the refusal to use it as intended.
- Maps cleanly to the TANK kit: high HP + slow = his mass and reluctance; defensive stance =
  his vow; knockback-shove = "get back, I will not hurt you, but you shall not pass." The kit
  IS the character.

## 4. THE BODY  ⏳ OPEN - awaiting Aviv's references
Locked shape-facts (derive from essence, safe now):
- **Colossal - the biggest silhouette in the roster**, heaviest read, low centre of gravity,
  planted stance. `huge <- because built as a guardian/weapon of scale.`
- **Cracked / damaged** - visible fractures from the destruction he caused; `cracks <- because
  he carries every bit of what he broke.`
- **Material / surface / exact form = OPEN.** Aviv rejected committing to my "ancient stone /
  marble" default and will bring references. Candidate essence-hook to watch for: **something
  gentle growing IN the cracks** (moss / flowers / new life) = the restoration he seeks, made
  literal on his body. Hold until refs.

## 5. THE SURFACE  ⏳ OPEN - awaiting references
- **Signature colour:** must be distinct from FANG (orange) and ZERO (blue). Direction to test
  against refs: earthy / green / bronze / weathered - NOT a hot or cold hero colour. `TBD.`
- **Signature story-object:** `TBD` from refs. Strong candidate: the life growing in his cracks,
  or a fragment of the thing he destroyed that he carries.
- **Face & default expression:** sorrowful, weary, gentle - **NOT rage.** A destroyer with kind,
  tired eyes. `expression <- because the contradiction (a weapon that grieves).`

## 6. THE VIBE IN MOTION  (feeds animation later - animation itself is DEFERRED)
- **Idle:** slow, heavy; slightly bowed / penitent, not a proud fighter's stance.
- **Signature gesture (his alone):** raises his arms to **shield**, not to strike.
- **Win:** does not celebrate - lowers his guard, relieved it's over.
- **Lose:** kneels / crumbles - a monument coming down.

## 7. ROSTER FIT + THE AXIS HE OWNS (LOCKED)
- **Owns the "immovable protector / penance" axis** - no other fighter is defensive-by-vow.
  FANG rushes in; ZERO controls from range; **ATLAS never chases and never kills - he endures.**
- Contrasts: vs FANG (tiny reckless speed vs immense patient mass); vs ZERO (cold distant control
  vs warm bodily presence). Fire / ice / stone. Three clean poles.

## 8. MECHANICS <-> FICTION COHERENCE (gate - PASS)
Every tank stat expresses him: high HP = endurance/penance; slow = mass + reluctance; defensive
stance = his vow; knockback-not-strike = "shove, don't kill." No stat contradicts the fiction.

## 9. VOICE
Slow, deep, few words, weary and gentle. His "taunts" are warnings or apologies, never threats.
- Warning: *"Turn back."* / *"I don't want to do this."*
- Win (quiet, sad): *"...Still standing. I'm sorry."*
- Lose: *"...Good. Let me rest."*
- Catchphrase / core line: *"You shall not pass - and neither will I strike."*

---

## THE ADVERSARY GATE (run before final lock)
1. **Cliche check:** closest existing = the "gentle giant" tank (many games). **Fresh angle:** not
   just gentle - *penitent*, and specifically a **guardian who destroyed his own charge** and
   fights only as atonement; the wish (restore what he destroyed) makes the whole kit a moral act,
   not just "big nice guy." Watch that the LOOK (pending refs) doesn't fall back to generic
   rock-golem - Aviv's references are the guard against that.
2. **Roster redundancy:** none - he owns the defensive-by-vow axis; FANG/ZERO are offense/control.
3. **Orphan traits:** none yet - surface is intentionally unbuilt, so nothing unearned is drawn.
   (Re-run this gate once the look locks against refs.)

## STATUS
- Essence LOCKED (Aviv, 2026-07-03): being = giant guardian statue that walked; wish = restore
  what he destroyed; name = ATLAS (bears the weight / the burden).
- **NEXT: Aviv brings visual references → shape material/palette/signature-object against them →
  re-run the Adversary Gate on the look → THEN generate (chibi-plus style, Tripo pipeline).**
  No generation before the references.
